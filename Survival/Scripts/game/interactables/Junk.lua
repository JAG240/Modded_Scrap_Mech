...
function Junk.server_onCreate( self )
	...
	if self.type == "shooter" then
		...
		self.maxForce = 135
		self.spreadDeg = 5.0
		if self.params and self.params.projectileName then
			self.projectileName = self.params.projectileName
		else
			self.projectileName = "tape"
		end
		if self.params and self.params.projectileName then
...
function Junk.server_onFixedUpdate( self, timeStep )
	...
	if self.type == "shooter" and self.projectileName then
		if self.shotsLeft > 0 then
			...
			if self.fireDelayProgress >= self.fireDelay then
				...
				sm.projectile.shapeProjectileAttack( self.projectileName, self.projectileDamage, firePos, dir * fireForce, self.shape )
