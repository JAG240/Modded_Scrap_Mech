...
function ElectricEngine.controlEngine( self, direction, active, timeStep, gearIdx )
	...
	self.motorVelocity = ( active and direction or 0 ) * self.power * self.level.gears[gearIdx].velocity
...
function ElectricEngine.server_onFixedUpdate( self, timeStep )
	...
	-- Control engine
	if self.batteryPoints > 0 or canSpend then
	...
	if self.dirtyClientTable then
		self.network:setClientData( { gearIdx = self.saved.gearIdx, engineHasFuel = self.hasBattery } )
		self.dirtyClientTable = false
	end
end
	...
	if self.engineHasFuel and not params.engineHasFuel then
		...
		if character then
			if ( self.shape.worldPosition - character.worldPosition ):length2() < 100 then
				sm.gui.displayAlertText( "#{INFO_OUT_OF_BATTERY}" )
			end
		end
	end
...
function ElectricEngine.client_onFixedUpdate( self, timeStep )
	...
	local active, direction, _, externalFuelTank, inputConnection = self:getInputs()
...
function ElectricEngine.client_onInteract( self, character, state )
	if state == true then
		...
		self.gui:setText( "Name", "E L E C T R I C  E N G I N E" )
		...
		local active, direction, steering, externaFuelContainer, inputConnection = self:getInputs()
...
function ElectricEngine.cl_updateEffect( self, direction, active )
	...
	if self.effect:isPlaying() == false and #bearings > 0 and math.abs( velocity ) > 0 then
		self.effect:start()
	elseif self.effect:isPlaying() and ( onLift or #bearings == 0 or velocity == 0 ) then
