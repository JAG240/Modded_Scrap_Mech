			...
			if sm.exists( result ) then
				...
				elseif type( result ) == "AreaTrigger" then
					...
					if userData and userData.water == true then
						local container = FindContainerToCollectTo( self.sv.connectedContainers, obj_consumable_water, 1 )
						if container then
							...
							if raycastStart.z > waterZ then
								...
								if hit and result.type == "areaTrigger" then
									table.insert( incomingObjects, { container = container, uuid = obj_consumable_water, amount = 1 } )
								end
							else
								table.insert( incomingObjects, { container = container, uuid = obj_consumable_water, amount = 1 } )
							end
						end
					elseif userData and userData.chemical == true then
						local container = FindContainerToCollectTo( self.sv.connectedContainers, obj_consumable_chemical, 1 )
						if container then
							...
							if raycastStart.z > waterZ then
								...
								local hit, result = sm.physics.raycast( raycastStart, raycastEnd, self.shape:getBody(), sm.physics.filter.static + sm.physics.filter.areaTrigger )
								if hit and result.type == "areaTrigger" then
									table.insert( incomingObjects, { container = container, uuid = obj_consumable_chemical, amount = 1 } )
								end
							else
								table.insert( incomingObjects, { container = container, uuid = obj_consumable_chemical, amount = 1 } )
							end
