...
function PackingStationScreen.sv_updateClientData( self, count, externalOpen )
	...
	local publicData = {
		...
		projectileName = dataset.projectileName,
		shape = dataset.shape,
...
function PackingStationScreen.sv_t_onProjectile( self, trigger, hitPos, hitTime, hitVelocity, projectileName )
	...
	if dataset.projectileName == projectileName then
					...
					for _,shape in ipairs( result:getShapes() ) do
						if shape:getShapeUuid() == obj_pneumatic_pump and not shape.interactable:isActive() then
							if (shape:getWorldPosition() - areaTriggerCenter):length() < 1.5 then
								...
								publicData.packingStationProjectileName = dataset.projectileName
...
function PackingStationScreen.cl_t_onProjectile( self, trigger, hitPos, hitTime, hitVelocity, projectileName )
