...
function Elevator.client_onCreate( self )
	self.moveEffect = sm.effect.createEffect( "ElevatorWall", self.interactable )
	self.musicEffect = sm.effect.createEffect( "ElevatorMusic", self.interactable )
	self.ambianceEffect = sm.effect.createEffect( "ElevatorAmbiance", self.interactable )
	self.musicEffect:start()
	self.ambianceEffect:start()
end
...
function Elevator.client_onUpdate( self, deltaTime )
	...
	--local distance = ( self.shape.worldPosition - sm.localPlayer.getPosition() ):length()
	if self.interactable.power == 0 then
		if self.moveEffect:isPlaying() == true then
			self.moveEffect:stop()
		end
	else
		if self.moveEffect:isPlaying() == false then
...
function Elevator.sv_panelButtonPressed( self )
	...
	if self.saved.portal:hasOpeningA() and self.saved.portal:hasOpeningB() then
		...
		self:createElevatorDestination()
		g_elevatorManager:sv_go( self.interactable )
	end
end
...
function Elevator.createElevatorDestination( self )
