...
function GrowingHarvestable.server_onMelee( self, hitPos, attacker, damage )
	if not self.sv.harvested and sm.exists( self.harvestable ) then
		...
		sm.harvestable.create( hvs_soil, sm.harvestable.getPosition( self.harvestable ), sm.harvestable.getRotation( self.harvestable ) )
		sm.harvestable.destroy( self.harvestable )
		self.sv.harvested = true
	end
end

function GrowingHarvestable.server_onProjectile( self, hitPos, hitTime, hitVelocity, projectileName, attacker, damage )
	...
	elseif projectileName == "fertilizer" then
		self:sv_performUpdate() -- Catch up to current time
		self.sv.fertilizer = true
		self:sv_performUpdate() -- Save and synch fertilizer
	elseif g_survivalDev and projectileName == "potato" then --HACK
		self:sv_performUpdate() -- Catch up to current time
		self.sv.growTicks = self.sv.growTickTime
		self:sv_performUpdate() -- Save and synch fertilizer
	...
	if not self.sv.harvested and sm.exists( self.harvestable ) then
		...
		if growFraction >= 1.0 then
			...
			if self.sv.matureUid then
				sm.harvestable.create( self.sv.matureUid, worldPos, worldRot )
			end
			sm.harvestable.destroy( self.harvestable )
			self.sv.harvested = true
			return
...
function GrowingHarvestable.sv_e_fertilize( self )
