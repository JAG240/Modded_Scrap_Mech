...
function LootHarvestable.server_onCreate( self )
	...
	self.network:setClientData( { uuid = self.sv.saved.uuid, quantity = self.sv.saved.quantity } )
end
...
function LootHarvestable.client_onClientDataUpdate( self, params )
	...
	elseif params.uuid == obj_outfitpackage_epic then
		...
		local size = sm.item.getShapeSize( self.cl.uuid )
	...
	self.cl.itemEffect:setParameter( "uuid", params.uuid )
	self.cl.itemEffect:setParameter( "Color", sm.shape.getShapeTypeColor( self.cl.uuid ) )
