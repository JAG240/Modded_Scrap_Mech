...
function PotatoShotgun.client_onEquip( self )
...
function PotatoShotgun.client_onUnequip( self )
...
function PotatoShotgun.cl_onPrimaryUse( self, state )
	...
	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		...
		if sm.container.canSpend( sm.localPlayer.getInventory(), obj_plantables_potato, 2 ) then
