...
function PotatoGatling.client_onEquip( self )
...
function PotatoGatling.client_onUnequip( self )
...
function PotatoGatling.cl_updateGatling( self, dt )
	...
	if self.fireCooldownTimer <= 0.0 and self.gatlingWeight >= 1.0 and self.gatlingActive then
		...
		if sm.container.canSpend( sm.localPlayer.getInventory(), obj_plantables_potato, 1 ) then
