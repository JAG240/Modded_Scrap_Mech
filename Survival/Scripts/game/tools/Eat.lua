...
function Eat.client_onUpdate( self, dt )
	...
	if self.tool:isLocal() then
		...
		-- Synch equipped renderables to other clients
		if self.activeItem ~= self.lastSentItem then
			self.lastSentItem = self.activeItem
...
function Eat.client_onEquip( self )
	if self.tool:isLocal() then
		self.activeItem = sm.localPlayer.getActiveItem()
		self:cl_updateEatRenderables()
		self:cl_loadAnimations()
...
function Eat.sv_n_updateEatRenderables( self, activeUid, player )
	self.network:sendToClients( "cl_n_updateActiveItem", { activeUid = activeUid, player = player } )
end

function Eat.cl_n_updateActiveItem( self, params )
	if params.player ~= sm.localPlayer.getPlayer() then
		self.activeItem = params.activeUid
		self:cl_updateEatRenderables()
		self:cl_loadAnimations()
	end
end
