...
function Bucket.sv_n_onUse( self, params )
	if params.projectileName == "water" then
	...
	elseif params.projectileName == "chemical" then
		sm.container.beginTransaction()
	...
	elseif params.projectileName == nil then
		sm.container.beginTransaction()
...
function Bucket.cl_n_onUse( self, params )
	if self.tool:isLocal() then
		if params.projectileName then
			local dir = sm.noise.gunSpread( params.dir, SpreadDeg )
			local fakePosSelf = self:calculateTpMuzzlePos()
			sm.projectile.projectileAttack( params.projectileName, 0, params.firePos, dir * FireVelocity, sm.localPlayer.getPlayer(), params.fakePos, fakePosSelf )
		end
	else
		self:onUse( params )
	end
		...
		if success then
			...
			if fill then
				...
				if primaryState == sm.tool.interactState.start then
					self:cl_fill( fill, fromContainer )
				end
				return true, false
			end
		end
...
function Bucket.cl_fill( self, fill, fromContainer )
	local params = { container = sm.localPlayer.getInventory(), selectedSlot = sm.localPlayer.getSelectedHotbarSlot(), fill = fill, fromContainer = fromContainer }
...
function Bucket.cl_throw( self, activeItem )
	...
	if self.fireCooldownTimer <= 0.0 then
		if sm.container.canSpend( sm.localPlayer.getInventory(), activeItem, 1 ) then
			...
			local params = { projectileName = projectileName, firePos = firePos, fakePos = fakePos, dir = dir, 
				container = sm.localPlayer.getInventory(), selectedSlot = sm.localPlayer.getSelectedHotbarSlot() }
			self.network:sendToServer( "sv_n_onUse", params )
			self:onUse( params )
