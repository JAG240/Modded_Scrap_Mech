...
function Glowstick.client_onEquippedUpdate( self, primaryState, secondaryState, forceBuildActive )
	...
	if self.pendingThrowFlag then
		...
		if time >= frameTime and frameTime ~= 0 then
			...
			local firePos = sm.localPlayer.getPosition() + sm.vec3.new( 0, 0, 0.5 )
			...
			print("fireVelocity", fireVelocity)
			local owner = self.tool:getOwner()
			if owner then
				sm.projectile.projectileAttack( "glowstick", 0, firePos, dir * fireVelocity, owner )
			end
