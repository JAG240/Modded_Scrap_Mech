...
function Planter.client_onEquippedUpdate( self, primaryState, secondaryState )
	...
	if soilHarvestable then
		if primaryState == sm.tool.interactState.start then
			...
			if plantable then
				local params = { targetSoil = soilHarvestable, plantableUuid = activeItem, plantedHarvestableUuid = sm.uuid.new( plantable.harvestable ), playerInventory = sm.localPlayer.getInventory() }
