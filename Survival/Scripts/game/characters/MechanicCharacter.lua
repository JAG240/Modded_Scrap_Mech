-- MechanicCharacter.lua --
dofile "Character.lua"


MechanicCharacter = class( Character )
...
function MechanicCharacter.server_onCreate( self )
	...
	if player then
		sm.event.sendToPlayer( player, "sv_onSpawnCharacter" )
	end
end
...
function MechanicCharacter.client_onGraphicsLoaded( self )
	...
	self.underWaterEffect = sm.effect.createEffect( "UnderwaterAmbiance" )
...
function MechanicCharacter.client_onUpdate( self, deltaTime )
	...
	if self.diveEffect then
		if self.character:isDiving() then
			...
			if self.isLocal and not self.underWaterEffect:isPlaying() then
				self.underWaterEffect:start()
			end
		elseif not self.character:isDiving() then
			...
			if self.isLocal and self.underWaterEffect:isPlaying() then
				self.underWaterEffect:stop()
			end
		end
	end
...
function MechanicCharacter.client_handleEvent( self, event )
	...
	if self.burningEffect then
		if self.graphicsLoaded then
			if event == "ignited" and not self.burningEffect:isPlaying() then
				self.burningEffect:start()
			elseif event == "doused" then
				self.burningEffect:stop()
			end
		end
	end
