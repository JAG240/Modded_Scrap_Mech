...
Overworld.terrainScript = "$SURVIVAL_DATA/Scripts/game/terrain/terrain_overworld.lua"
...
function Overworld.sv_spawnNewCharacter( self, params )
	...
	if #nodes > 0 then
		...
		local spawnDirection = spawnNode.rotation * sm.vec3.new( 0, 1, 0 )
...
function Overworld.server_onCellLoaded( self, x, y )
	...
	g_elevatorManager:sv_loadElevatorsOnCell( x, y, tags )
...
function Overworld.server_onCellReloaded( self, x, y )
...
function Overworld.server_onProjectile( self, hitPos, hitTime, hitVelocity, projectileName, attacker, damage, userData )
	--print( "Projectile hit in Overworld! "..tostring( hitPos ) )
	--print( hitTime )
	--print( hitVelocity )
	...
	--print( damage )
	--print( userData )
	BaseWorld.server_onProjectile( self, hitPos, hitTime, hitVelocity, projectileName, attacker, damage, userData )

	if self.enablePathPotatoes and projectileName == "potato" then
...
function Overworld.sv_ambush( self, params )
	...
	local groupId = g_unitManager:sv_getDailyGroupId()
		...
		if player.character and player.character:getWorld() == self.world then
			...
			if validNodesCount > 0 then
				...
				for i = 1, #incomingUnits do
					...
					sm.unit.createUnit( incomingUnits[i], unitPos + sm.vec3.new( 0, 0.1, 0), yaw, { temporary = true, roaming = true, ambush = true, tetherPoint = playerPosition, groupId = groupId } )
				end
			else
				...
				for i = 1, #incomingUnits do
					...
					while spawnAttempts < maxSpawnAttempts do
						...
						if success and ( result.type == "invalid" or result.type == "terrainSurface" ) then
							...
							sm.unit.createUnit( incomingUnits[i], spawnPosition, yaw, { temporary = true, roaming = true, ambush = true, tetherPoint = playerPosition, groupId = groupId } )
							break
						end
					end
				end
	...
	for i = 1, #incomingUnits do
		...
		while spawnAttempts < maxSpawnAttempts do
			...
			if success and ( result.type == "invalid" or result.type == "terrainSurface" ) then
				...
				sm.unit.createUnit( incomingUnits[i], unitPos, yaw, { temporary = true, roaming = true, raider = true, tetherPoint = attackPos, groupId = groupId, deathTick = deathTick } )
				break
			end
		end
	end
...
function Overworld.sv_e_spawnTempUnitsOnCell( self, params )
	...
	if validNodesCount > 0 then
		...
		for i = 1, #incomingUnits do
			...
			sm.unit.createUnit( incomingUnits[i], unitPos + sm.vec3.new( 0, 0.1, 0), yaw, { temporary = true, groupId = groupId } )
		end
	else
		local maxSpawnAttempts = 32
		for i = 1, #incomingUnits do
			...
			while spawnAttempts < maxSpawnAttempts do
				...
				if success and ( result.type == "invalid" or result.type == "terrainSurface" ) then
					...
					local yaw = math.atan2( direction.y, direction.x ) - math.pi / 2
					spawnPosition = result.pointWorld
					sm.unit.createUnit( incomingUnits[i], spawnPosition, yaw, { temporary = true, groupId = groupId } )
					break
				end
			end
		end
