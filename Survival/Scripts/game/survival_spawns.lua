...
local function CreateUnit( tag, node, cell )
	...
	local unit
	if tag == "HAYBOT" then
		unit = sm.unit.createUnit( unit_haybot, node.position, toYaw( node.rotation ), { tetherPoint = node.position, groupId = cell.groupId, deathTick = sm.game.getCurrentTick() + UnitTicksBetweenRespawns[tag] + 400  } )
	elseif tag == "TOTEBOT_GREEN" then
		unit = sm.unit.createUnit( unit_totebot_green, node.position, toYaw( node.rotation ), { tetherPoint = node.position, groupId = cell.groupId, deathTick = sm.game.getCurrentTick() + UnitTicksBetweenRespawns[tag] + 400 } )
	elseif tag == "FARMBOT" then
		unit = sm.unit.createUnit( unit_farmbot, node.position, toYaw( node.rotation ), { tetherPoint = node.position,groupId = cell.groupId, deathTick = sm.game.getCurrentTick() + UnitTicksBetweenRespawns[tag] + 400 } )
	elseif tag == "TAPEBOT" then
		unit = sm.unit.createUnit( g_tapebots[math.random( 1, #g_tapebots )], node.position, toYaw( node.rotation ), { tetherPoint = node.position, groupId = cell.groupId, deathTick = sm.game.getCurrentTick() + UnitTicksBetweenRespawns[tag] + 400 } )
	elseif tag == "WOC" then
...
local function SpawnUnits( cell, nodes, spawnMax, storage, tag )
	...
	shuffle( nodes )
	cell.groupId = cell.groupId or g_unitManager:sv_generateNextGroupId()
	local spawnCount = 0
...
function RespawnFromNodeOnCellReloaded( cell, tag )
	...
	assert( currentTick >= loaded.tick )
