...
-- p = progress from first point (1) to last point (#linePoints)
function spline( points, p, distances )
	assert( #points > 1, "Must have at least 2 points in pointList" )
...
function lerpDirection( fromDirection, toDirection, p )
	...
	while cameraDesiredHeading > math.pi * 2 do
		cameraDesiredHeading = cameraDesiredHeading - math.pi * 2
	end
	while cameraDesiredHeading < 0 do
		cameraDesiredHeading = cameraDesiredHeading + math.pi * 2
	end

	local cameraHeading = math.atan2( -fromDirection.x, fromDirection.y )
	local cameraPitch = math.asin( fromDirection.z )
	while cameraHeading >= math.pi * 2 do
		cameraHeading = cameraHeading - math.pi * 2
		cameraDesiredHeading = cameraDesiredHeading - math.pi * 2
	end
	while cameraHeading < 0 do
		cameraHeading = cameraHeading + math.pi * 2
		cameraDesiredHeading = cameraDesiredHeading + math.pi * 2
	end
